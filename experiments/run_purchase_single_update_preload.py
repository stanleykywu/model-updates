...
def run_argparse():
    ...
    parser.add_argument("--epochs_pre", nargs="?", const=95, type=int, default=95)
...
def main(args):
    ...
    all_dses = data_utils.purchase(1, args.n, args.nup, data_seed=3000)
    ...
    model.fit_sgd(x_trn, np.eye(100)[y_trn], epochs=args.epochs_pre)
    ...
    for update_ds, update_test in zip(update_dses, update_tests):
        ...
        if args.retrain == "sgd":
            ...
            model.fit_sgd(x_up, np.eye(100)[y_up], epochs=args.epochs_re, sgd_only=True)
        else:
    ...
    save_tag = f"experiments/purchase/single_update/{args.n}_{args.nup}_{args.retrain}"
