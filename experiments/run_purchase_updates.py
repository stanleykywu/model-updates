...
def run_argparse():
    ...
    parser.add_argument("--lr", nargs="?", const=0.1, type=int, default=0.1)
...
def main(args):
    ...
    for data_seed in range(args.num_trials):
        for i in range(args.num_models):
            model = model_utils.ImageNNet("2f", 100, 100, np.arange(100), args.nup)
            load_model = tf.keras.models.load_model(
                "experiments/purchase/saved_model/model_{}".format(i)
            )
            model.keras_model = load_model
            model.lr = args.lr

            all_dses = pickle.load(
                open("experiments/purchase/saved_model/dataset_{}".format(i), "rb")
            )
            ...
            accuracies[0]["training"].append(model.score(x_trn, np.eye(100)[y_trn]))
            accuracies[0]["testing"].append((model.score(x_tst, np.eye(100)[y_tst])))
            ...
            for index, (update_ds, update_test) in enumerate(
                ...
                for pt_id, loss in enumerate(
                    np.array(
                        [
                            model.get_loss(np.array([x]), np.array([np.eye(100)[y]]))
            ...
            for index, (update_ds, update_test) in enumerate(
                ...
                accuracies[index + 1]["training"].append(
                    model.score(x_trn, np.eye(100)[y_trn])
                )
                accuracies[index + 1]["testing"].append(
                    model.score(x_tst, np.eye(100)[y_tst])
                )
    ...
    np.save(
        f"experiments/purchase/multiple_update/{args.num_updates}_{args.nup}_{args.retrain}",
